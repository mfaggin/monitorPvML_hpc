...
void filterTree(std::string inputsNames, std::string myCase, bool isMC, bool do_cout, bool useOnlyDFdebugFromRun520259LHC23d1e) {
    ...
    int counterDF_treePreparation = -1;
    for(int i=0; i<static_cast<int>(files.size()); i++) {
        ...
        for(int iDF=0; iDF < listDF->GetEntries(); iDF++) {
            ...
            /// update immediately the DF counter (defined to -1 at the beginning)
            counterDF_treePreparation++;
            ...
            ///
            if(useOnlyDFdebugFromRun520259LHC23d1e && strDir.compare("DF_2234495402005329")) {
                std::cout << "\tdirectory " << strDir << " skipped (!!! TEMPORARY !!! Just for debug !!!)" << std::endl;
                continue;
            }
            ///
            /// !!! TEMPORARY !!! Just for debug !!!
            ...
            // apparently, changing cycle the init function is recalled and the counter set to 0
            ...
            if (do_cout){
                ...
                for(std::size_t j=0; j < vecGlobBcInRun.size(); j++) {
            ...
            for(Long64_t i=0; i < inputTree->GetEntries(); i++) {
                ...
                /*
                /// WRONG!
                /// Apparently we cannot say a priori that duplicate PV are those belonging to the same BC.
                /// It might be that 2 or more PV belonging to the same BC might simply have a wrong time association
                /// See README.txt for the findings.
                ...
                */
                if( isMC && ( countCollIdMc > 1 ) ) {
                    /// This is a collision that shares the same MC collision with (at least) another collision in the same DF
                    /// (i.e. same collisionId, which is unique within each DF)
                    isCurrentDuplicate = 1;
                }
                ...
                counterDF = counterDF_treePreparation; //counterDF_input;
    ...
    fout.cd();
    finalTree.Write();
    std::cout << ">>> TTree with " << finalTree.GetEntries() << " entries saved into file." << std::endl;
    sleep(3);
    fout.Close();
